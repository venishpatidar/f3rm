"""
High Resolution DINO V2 ViT Feature Extractor
...
class ViTExtractor:
    ...
    def __init__(self, model_type: str = 'dinov2_vits14', stride: int = 4, model: nn.Module = None,
    ...
    def preprocess(self, image_path: Union[str, Path],
        ...
        # Calculating the new width that is a multiple 14 and height = load_size
        width, height = pil_image.size
        new_width = int((width/height)*load_size)
        new_width = new_width + (14 - new_width % 14) % 14
        if load_size is not None:
            pil_image = transforms.Resize((load_size,new_width), interpolation=transforms.InterpolationMode.LANCZOS)(pil_image)
